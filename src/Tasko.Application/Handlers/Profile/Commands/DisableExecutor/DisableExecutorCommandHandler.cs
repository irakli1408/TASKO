...
public sealed class DisableExecutorCommandHandler : IRequestHandler<DisableExecutorCommand, MyProfileDto>
{
    private readonly ITaskoDbContext _db;
    private readonly ICurrentStateService _current;
    private readonly IMediator _mediator;

    public DisableExecutorCommandHandler(ITaskoDbContext db, ICurrentStateService current, IMediator mediator)
    {
        _db = db;
        _current = current;
        _mediator = mediator;
    ...
    public async Task<MyProfileDto> Handle(DisableExecutorCommand request, CancellationToken ct)
    {
        var userId = _current.GetUserIdLong();

        var user = await _db.Users.FirstAsync(x => x.Id == userId, ct);

        user.DisableExecutor();
